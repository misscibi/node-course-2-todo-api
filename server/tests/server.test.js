...
describe('PATCH /todos/:id', () => {
    ...
    it('should not update the todo from another user', (done) => {
        var hexId = todos[1]._id.toHexString();
        var todoDelta = {text: 'test suite update intruder', completed: true};
        ...
        request(app)
            ...
            .end((error, response) => {
                ...
                Todo.findById(hexId).then((todo) => {
                    expect(todo.text).not.toBe(todoDelta.text);
                    expect(todo.completed).not.toBe(todoDelta.completed);
                    expect(typeof todo.completedAt).toBeFalsy();
                    done();
                }).catch((error) => done(error));
            });
    });
